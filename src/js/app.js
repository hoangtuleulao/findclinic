  ...
  init: function() {
    ...
    $.getJSON('../clinics.json', function(data) {
      ...
      for (i = 0; i < data.length; i ++) {
        ...
        clinicTemplate.find('.clinic-status').text(data[i].breed);
        clinicTemplate.find('.clinic-price').text(data[i].age);
        clinicTemplate.find('.clinic-location').text(data[i].location);
        clinicTemplate.find('.btn-adopt').attr('data-id', data[i].id);
    ...
    App.contracts.Adoption.deployed().then(function(instance) {
      ...
      for (i = 0; i < adopters.length; i++) {
        if (adopters[i] !== '0x0000000000000000000000000000000000000000') {
          $('.panel-clinic').eq(i).find('button').text('Success').attr('disabled', true);
        }
      }
    }).catch(function(err) {
